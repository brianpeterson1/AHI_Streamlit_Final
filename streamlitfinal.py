...
## simple bar chart display of hospital types and ownership types in NY
...
##Analysis of Stony Brook Hospital using hospital data
...
## Overall Description of Stony Brook Hospital using some outpatient data..
...
##Now, we will use the .describe function to help give us an overall view and comparison of NYU/Stony for select variables..
...
##Common discharges for NYU Lagone and Stony Brook DATA
##NYU First

NYU_Lagone_medical_inpatient = df_inpatient_2[df_inpatient_2['provider_name'] == 'NYU HOSPITALS CENTER']

st.subheader('We will now show the most common DRG definitions along with total discharges per the conditions and average medicare/total payments.')
...
st.markdown('It appears that the most common discharges were for a DRG 470, which is major joint replacement. The most expensive medicare and total payments were for a DRG 003,  ECMO OR TRACH W MV >96 HRS OR PDX EXC FACE, MOUTH & NECK W MAJ O.R.')
...
st.header('DRGs for Stony Brook and NYU')
...
st.subheader('Top 5 DRGs in Stony Brook')
figtopstony = px.pie(top5stony, values='total_discharges', names='drg_definition')
st.plotly_chart(figtopstony)

st.subheader('Top 5 DRGs in NYU')
figtopnyu = px.pie(top5nyu, values='total_discharges', names='drg_definition')
st.plotly_chart(figtopnyu)
st.markdown('The two pie charts above help visually display the DRG data for NYU/STONY data in prior tables, showing pie charts for only the top DRGS. (Side note- may need to press the expand option on top right of chart to see it properly. For some reason, the legend is blocking the table. When you expand it with the option, you can see everything fine).')

##need more data on visuals and such, otherwise looks good!
## lets take a look at the hospital information for NYU Lagone first
## now to make a side by side chart
...
##we will now make visualizations for DRG definition and total discharges.. first for NYU
